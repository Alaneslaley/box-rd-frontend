...
@Component({ selector: 'app-student-detail-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (student(); as current) {
    <app-page-header [title]="current.fullName" description="Detalle básico del expediente del alumno."><div class="header-actions"><a class="btn btn-secondary" routerLink="/students">Volver</a>@if (canEdit) { <a class="btn btn-primary" [routerLink]="['/students', current.id, 'edit']">Editar</a> }</div></app-page-header>
