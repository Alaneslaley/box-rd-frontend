...
@Component({ selector: 'app-student-detail-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (student(); as current) {
    ...
    <div class="detail-grid"><section class="card detail-card"><h2>Datos personales</h2><dl><div><dt>Estado</dt><dd><app-status-badge [label]="statusLabel(current.status)" [tone]="statusTone(current.status)" /></dd></div><div><dt>Nivel</dt><dd>{{ levelLabel(current.level) }}</dd></div><div><dt>Fecha de nacimiento</dt><dd>{{ current.birthDate }}</dd></div><div><dt>Edad</dt><dd>{{ current.age }} años · {{ current.ageCategory }}</dd></div><div><dt>Correo</dt><dd>{{ current.email || 'No registrado' }}</dd></div><div><dt>Teléfono</dt><dd>{{ current.phone || 'No registrado' }}</dd></div><div><dt>Objetivo personal</dt><dd>{{ current.personalGoal || 'No registrado' }}</dd></div></dl></section>
      <section class="card detail-card"><h2>Tutor</h2>@if (current.guardian; as guardian) { <dl><div><dt>Nombre</dt><dd>{{ guardian.firstName }} {{ guardian.lastName }}</dd></div><div><dt>Relación</dt><dd>{{ guardian.relationship }}</dd></div><div><dt>Teléfono</dt><dd>{{ guardian.phone }}</dd></div><div><dt>Correo</dt><dd>{{ guardian.email || 'No registrado' }}</dd></div></dl> } @else { <p>No hay tutor registrado.</p> }</section>
    ...
    <div class="placeholder-grid"><section class="state-card"><h2>Membresía actual</h2><p>Pendiente para sprint posterior.</p></section><section class="state-card"><h2>Asistencia reciente</h2><p>Pendiente para sprint posterior.</p></section><section class="state-card"><h2>Seguimiento deportivo</h2><p>Pendiente para sprint posterior.</p></section></div>
  }` })
export class StudentDetailPageComponent implements OnInit {
  private readonly facade = inject(StudentsFacade); private readonly route = inject(ActivatedRoute); private readonly session = inject(AuthSessionStore);
  ...
  ngOnInit(): void { const id = this.route.snapshot.paramMap.get('id') ?? ''; this.facade.loadDetail(id).pipe(finalize(() => this.loading.set(false))).subscribe({ next: (student) => this.student.set(student), error: (error: ApiError) => { this.error.set(studentErrorMessage(error, 'No fue posible cargar el alumno.')); this.traceId.set(error.traceId); } }); }
