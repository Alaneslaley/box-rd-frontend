...
@Component({ selector: 'app-student-detail-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (student(); as current) {
    ...
    <section class="card student-memberships"><div class="section-heading"><div><h2>Membresías</h2><p>Historial de vigencias informado por el backend.</p></div>@if (canCreateMembership) { <a class="btn btn-primary" routerLink="/memberships/new" [queryParams]="{ studentId: current.id }">Nueva membresía</a> }</div>
