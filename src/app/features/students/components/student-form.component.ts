  ...
  template: `
    <form class="student-form" [formGroup]="form" (ngSubmit)="submitForm()" novalidate>
      <section class="card form-section"><h2>Datos personales</h2><div class="form-grid">
        ...
        <div class="form-field"><label for="phone">Teléfono celular *</label><input id="phone" type="tel" inputmode="tel" formControlName="phone" maxlength="25" autocomplete="tel" placeholder="Ej. 5551234567" />@if (invalid('phone')) { @if (form.controls.phone.hasError('required')) { <span class="field-error">El teléfono celular es obligatorio.</span> } @else if (form.controls.phone.hasError('maxlength')) { <span class="field-error">El teléfono celular no puede exceder 25 caracteres.</span> } @else { <span class="field-error">El teléfono celular no tiene un formato válido.</span> } }</div>
      ...
      <section class="card form-section" formGroupName="guardian"><h2>Tutor opcional</h2><p>Si capturas un tutor, completa su nombre, teléfono y relación.</p><div class="form-grid">
        ...
        <div class="form-field"><label for="guardianEmail">Correo del tutor</label><input id="guardianEmail" type="email" formControlName="email" maxlength="254" autocomplete="email" /></div>
...
export class StudentFormComponent {
  ...
  readonly form = new FormGroup({
    ...
    phone: new FormControl('', { nonNullable: true, validators: [Validators.required, Validators.pattern(PHONE_PATTERN), Validators.maxLength(25)] }),
  ...
  constructor() {
    effect(() => {
      ...
      if (!student) return;
      this.form.reset({ firstName: student.firstName, lastName: student.lastName, birthDate: student.birthDate, phone: student.phone ?? '', personalGoal: student.personalGoal ?? '', level: student.level,
  ...
  submitForm(): void {
    ...
    this.submitted.emit({ firstName: value.firstName.trim(), lastName: value.lastName.trim(), birthDate: value.birthDate, phone: value.phone.trim(),
