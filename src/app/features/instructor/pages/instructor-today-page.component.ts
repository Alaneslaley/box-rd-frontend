...
@Component({ selector: 'app-instructor-today-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, EmptyStateComponent, ErrorStateComponent, StatusBadgeComponent, ProtectedMediaImageComponent], template: `
  <app-page-header title="Instructor · Hoy" description="Vista rápida de los alumnos presentes en la jornada actual."><button class="btn btn-secondary" type="button" (click)="facade.loadToday()">Actualizar</button></app-page-header>
  ...
  @else if (!facade.page()?.content?.length) { <app-empty-state title="Aún no hay alumnos presentes" description="Los alumnos aparecerán cuando registren su asistencia." /> }
