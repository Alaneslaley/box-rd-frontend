...
@Component({ selector: 'app-cash-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, EmptyStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (facade.current(); as current) { <section class="card detail-card finance-detail"><div class="section-heading"><div><h2>Caja actual</h2><p>{{ display.cashRegisterLabel() }}</p></div>@if (canClose) { <a class="btn btn-primary" routerLink="/cash/close">Cerrar caja</a> }</div><dl><div><dt>Estado</dt><dd><app-status-badge [label]="statusLabel(current.status)" [tone]="statusTone(current.status)" /></dd></div><div><dt>Sucursal</dt><dd>{{ display.branchLabel(current.branchId) }}</dd></div><div><dt>Apertura</dt><dd>{{ dateTime(current.openedAt) }}</dd></div><div><dt>Abierta por</dt><dd>{{ display.userName(current.openedBy) }}</dd></div><div><dt>Caja inicial</dt><dd>{{ money(current.initialCash, current.currency) }}</dd></div><div><dt>Caja esperada</dt><dd>{{ money(current.expectedCash, current.currency) }}</dd></div><div><dt>Moneda</dt><dd>{{ current.currency }}</dd></div>@if (current.difference !== null) { <div><dt>Diferencia</dt><dd>{{ money(current.difference, current.currency) }}</dd></div> }@if (current.notes) { <div><dt>Notas</dt><dd>{{ current.notes }}</dd></div> }</dl></section> }
` })
