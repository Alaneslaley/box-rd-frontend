...
@Component({ selector: 'app-cash-open-form', imports: [ReactiveFormsModule], template: `
  <form class="student-form" [formGroup]="form" (ngSubmit)="submitForm()" novalidate><section class="card form-section"><h2>Apertura de caja</h2><p>La sucursal se toma de la sesión cuando está disponible.</p><div class="form-grid">
    <div class="form-field form-field-wide"><label for="cash-branch">Sucursal (opcional)</label><input id="cash-branch" formControlName="branchId" [readOnly]="initialBranchId() !== ''" placeholder="Se usará la sucursal de la sesión" />@if (invalid('branchId')) { <span class="field-error">Ingresa una sucursal válida.</span> }</div>
