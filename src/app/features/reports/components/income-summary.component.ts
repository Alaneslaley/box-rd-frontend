...
@Component({ selector: 'app-income-summary', imports: [EmptyStateComponent], template: `
  <section class="dashboard-section"><div class="section-heading"><div><h2>Ingresos de hoy</h2><p>Totales por moneda informados por el backend.</p></div></div>
    ...
    @else { <app-empty-state title="Sin ingresos para mostrar" description="El backend no devolvió totales de ingresos para la fecha operativa." /> }
  </section>
  <section class="dashboard-section"><div class="section-heading"><div><h2>Ingresos por método</h2><p>Desglose real de pagos por método y moneda.</p></div></div>
    ...
    @else { <app-empty-state title="Sin desglose por método" description="El backend no devolvió pagos agrupados por método para esta fecha." /> }
  </section>
` })
