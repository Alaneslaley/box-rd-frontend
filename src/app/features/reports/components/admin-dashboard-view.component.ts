...
@Component({ selector: 'app-admin-dashboard-view', imports: [KpiCardComponent, IncomeSummaryComponent], template: `
  <section class="dashboard-context" aria-label="Contexto del reporte"><span>Fecha operativa: <strong>{{ date(data().businessDate) }}</strong></span><span>Sucursal: <strong>{{ data().branchId }}</strong></span></section>
  <section class="kpi-grid" aria-label="Indicadores administrativos">
    <app-kpi-card title="Alumnos activos" [value]="data().activeStudents" subtitle="Expedientes activos informados por backend" tone="success" [route]="canStudents ? '/students' : null" />
    ...
    <app-kpi-card title="Asistencias de hoy" [value]="data().attendanceToday" subtitle="Check-ins confirmados por backend" tone="primary" [route]="canAttendance ? '/attendance' : null" />
  </section>
