...
@Component({ selector: 'app-user-detail-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (user(); as current) { <app-page-header [title]="current.firstName + ' ' + current.lastName" description="Consulta la información de la cuenta."><div class="header-actions"><a class="btn btn-secondary" routerLink="/security/users">Volver</a>@if (canRoles) { <a class="btn btn-primary" [routerLink]="['/security/users', current.id, 'roles']" [state]="{ user: current }">Cambiar roles</a> }@if (canStatus) { <a class="btn btn-secondary" [routerLink]="['/security/users', current.id, 'status']" [state]="{ user: current }">Cambiar estado</a> }</div></app-page-header>
    <section class="card detail-card security-detail"><dl><div><dt>Correo</dt><dd>{{ current.email }}</dd></div><div><dt>Sucursal</dt><dd>{{ display.branchLabel(current.branchName) }}</dd></div><div><dt>Estado</dt><dd><app-status-badge [label]="statusLabel(current.status)" [tone]="statusTone(current.status)" /></dd></div><div><dt>Versión de autorización</dt><dd>{{ current.authzVersion }}</dd></div><div><dt>Contraseña</dt><dd><app-status-badge [label]="passwordLabel(current.mustChangePassword)" [tone]="passwordTone(current.mustChangePassword)" /></dd></div><div><dt>Roles</dt><dd><div class="chip-row">@for (role of current.roles; track role) { <span class="role-chip">{{ role }}</span> }</div></dd></div><div class="detail-wide"><dt>Permisos asociados</dt><dd><div class="chip-row">@for (permission of current.permissions; track permission) { <span class="permission-chip">{{ permission }}</span> }@empty { <span class="muted">Sin permisos asociados para esta cuenta.</span> }</div></dd></div></dl></section>
  }
` })
export class UserDetailPageComponent implements OnInit {
  private readonly facade = inject(SecurityFacade); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router); private readonly session = inject(AuthSessionStore); readonly display = inject(DisplayNameResolverService);
