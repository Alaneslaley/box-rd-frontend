...
@Component({ selector: 'app-security-home-page', imports: [RouterLink, PageHeaderComponent], template: `
  ...
  <section class="state-card security-notice"><h2>Autorización de backend</h2><p>La autorización final, las contraseñas, los roles efectivos y la auditoría se validan en backend. Estas opciones solo mejoran la experiencia de uso.</p></section>
  <section class="security-home-grid">
    ...
    @if (canReadRoles) { <a class="card security-home-card" routerLink="/security/roles"><h2>Roles</h2><p>Consulta los roles y permisos publicados por el backend.</p><span class="btn btn-link">Abrir roles</span></a> }
  </section>
` })
