...
@Component({ selector: 'app-user-status-page', imports: [RouterLink, ReactiveFormsModule, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (user(); as current) { <app-page-header title="Cambiar estado" [description]="current.firstName + ' ' + current.lastName + ' · ' + current.email"><a class="btn btn-secondary" [routerLink]="['/security/users', current.id]" [state]="{ user: current }">Cancelar</a></app-page-header>
...
export class UserStatusPageComponent implements OnInit {
  ...
  save(): void { const current = this.user(); if (this.saving() || !current || this.form.invalid) { this.form.markAllAsTouched(); return; } this.saving.set(true); this.saveError.set(null); this.facade.updateUserStatus(current.id, { status: this.form.controls.status.value }).pipe(finalize(() => this.saving.set(false))).subscribe({ next: () => void this.router.navigate(['/security/users'], { state: { successMessage: 'Estado actualizado correctamente.' } }), error: (error: ApiError) => { this.saveError.set(securityErrorMessage(error, 'No fue posible actualizar el estado.')); this.traceId.set(error.traceId); } }); }
