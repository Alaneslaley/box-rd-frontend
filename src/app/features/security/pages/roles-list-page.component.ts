...
@Component({ selector: 'app-roles-list-page', imports: [PageHeaderComponent, LoadingStateComponent, EmptyStateComponent, ErrorStateComponent], template: `
  <app-page-header title="Roles" description="Consulta los roles y permisos disponibles."><button class="btn btn-secondary" type="button" (click)="facade.loadRoles()">Refrescar</button></app-page-header>
  ...
  @else if (!facade.roles().length) { <app-empty-state title="No hay roles disponibles" description="No hay roles disponibles para esta sesión." /> }
