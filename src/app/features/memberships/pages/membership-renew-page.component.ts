...
@Component({ selector: 'app-membership-renew-page', imports: [ReactiveFormsModule, PageHeaderComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  <app-page-header title="Renovar membresía" description="Indica la fecha efectiva de la renovación." />
  @if (!membership()) { <app-error-state title="No se pudo abrir la renovación" message="Inicia esta operación desde el listado de membresías."><button class="btn btn-secondary" type="button" (click)="cancel()">Volver al listado</button></app-error-state> }
  @else if (membership(); as current) {
    <section class="card renewal-summary"><h2>{{ current.planName }}</h2><dl><div><dt>Alumno</dt><dd>{{ display.studentName(current.studentName) }}</dd></div><div><dt>Sucursal</dt><dd>{{ display.branchLabel(current.branchName) }}</dd></div><div><dt>Vigencia actual</dt><dd>{{ current.startDate }} a {{ current.endDate }}</dd></div><div><dt>Estado</dt><dd><app-status-badge [label]="statusLabel(current.status)" [tone]="statusTone(current.status)" /></dd></div></dl></section>
    @if (error(); as message) { <app-error-state title="No fue posible renovar la membresía" [message]="message" [traceId]="traceId()" /> }
    <form class="card renewal-form" [formGroup]="form" (ngSubmit)="save()" novalidate><div class="form-field"><label for="effective-on">Fecha efectiva *</label><input id="effective-on" type="date" formControlName="effectiveOn" />@if (form.controls.effectiveOn.touched && form.controls.effectiveOn.invalid) { <span class="field-error">Ingresa una fecha en formato válido.</span> }</div><p>La nueva vigencia se mostrará al confirmar la renovación.</p><div class="form-actions"><button class="btn btn-secondary" type="button" [disabled]="saving()" (click)="cancel()">Cancelar</button><button class="btn btn-primary" type="submit" [disabled]="saving()">{{ saving() ? 'Renovando…' : 'Confirmar renovación' }}</button></div></form>
  }` })
