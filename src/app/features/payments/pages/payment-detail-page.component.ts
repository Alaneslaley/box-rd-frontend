...
@Component({ selector: 'app-payment-detail-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (payment(); as current) {
    <app-page-header [title]="'Pago ' + current.folio" description="Detalle informado por el backend." phase="Sprint 4"><div class="header-actions"><a class="btn btn-secondary" routerLink="/payments">Volver</a>@if (canReceipt) { <a class="btn btn-primary" [routerLink]="['/payments', current.id, 'receipt']">Ver recibo</a> }</div></app-page-header>
    @if (registeredSuccessfully) { <div class="alert alert-success" role="status">Pago registrado correctamente. El folio y el importe fueron generados por el backend.</div> }
    <section class="card detail-card finance-detail"><dl><div><dt>Folio</dt><dd>{{ current.folio }}</dd></div><div><dt>Estado</dt><dd><app-status-badge [label]="statusLabel(current.status)" [tone]="statusTone(current.status)" /></dd></div><div><dt>Alumno</dt><dd><a [routerLink]="['/students', current.studentId]">{{ current.studentId }}</a></dd></div><div><dt>Membresía</dt><dd>{{ current.membershipId }}</dd></div><div><dt>Caja</dt><dd>{{ current.cashRegisterId || 'No aplica para este método' }}</dd></div><div><dt>Método</dt><dd>{{ methodLabel(current.method) }}</dd></div><div><dt>Concepto</dt><dd>{{ conceptLabel(current.concept) }}</dd></div><div><dt>Importe</dt><dd>{{ money(current.amount, current.currency) }}</dd></div><div><dt>Moneda</dt><dd>{{ current.currency }}</dd></div><div><dt>Registrado</dt><dd>{{ dateTime(current.registeredAt) }}</dd></div></dl></section>
  }` })
