...
@Component({ selector: 'app-payment-detail-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  ...
  @else if (payment(); as current) {
    ...
    <section class="card detail-card finance-detail"><dl><div><dt>Folio</dt><dd>{{ current.folio }}</dd></div><div><dt>Estado</dt><dd><app-status-badge [label]="statusLabel(current.status)" [tone]="statusTone(current.status)" /></dd></div><div><dt>Alumno</dt><dd><a [routerLink]="['/students', current.studentId]">{{ display.studentName(current.studentId) }}</a></dd></div><div><dt>Membresía</dt><dd>{{ display.membershipLabel() }}</dd></div><div><dt>Caja</dt><dd>{{ current.cashRegisterId ? display.cashRegisterLabel() : 'No aplica para este método' }}</dd></div><div><dt>Método</dt><dd>{{ methodLabel(current.method) }}</dd></div><div><dt>Concepto</dt><dd>{{ conceptLabel(current.concept) }}</dd></div><div><dt>Importe</dt><dd>{{ money(current.amount, current.currency) }}</dd></div><div><dt>Moneda</dt><dd>{{ current.currency }}</dd></div><div><dt>Registrado</dt><dd>{{ dateTime(current.registeredAt) }}</dd></div></dl></section>
  }` })
export class PaymentDetailPageComponent implements OnInit {
  ...
  ngOnInit(): void { this.facade.loadPaymentDetail(this.id).pipe(finalize(() => this.loading.set(false))).subscribe({ next: (payment) => { this.payment.set(payment); this.display.preloadStudentNames([payment.studentId]); }, error: (error: ApiError) => { this.error.set(paymentErrorMessage(error, 'No fue posible cargar el pago.')); this.traceId.set(error.traceId); } }); }
