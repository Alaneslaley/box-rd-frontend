...
export class PaymentReceiptPageComponent implements OnInit {
  ...
  ngOnInit(): void { this.facade.loadReceipt(this.paymentId).pipe(finalize(() => this.loading.set(false))).subscribe({ next: (receipt) => { this.receipt.set(receipt); this.display.preloadStudentNames([receipt.studentId]); }, error: (error: ApiError) => { this.error.set(paymentErrorMessage(error, 'No fue posible cargar el recibo.')); this.traceId.set(error.traceId); } }); }
