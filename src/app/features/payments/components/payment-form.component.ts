...
@Component({
  ...
  template: `
    <form class="student-form" [formGroup]="form" (ngSubmit)="submitForm()" novalidate>
      <section class="card form-section"><h2>Datos del pago</h2><p>El importe, concepto, folio y recibo serán determinados por el backend.</p><div class="form-grid">
        <div class="form-field form-field-wide"><label for="payment-membership">Membresía *</label><select id="payment-membership" formControlName="membershipId"><option value="">Selecciona una membresía</option>@if (showExternalMembership()) { <option [value]="initialMembershipId()">Membresía indicada · {{ initialMembershipId() }}</option> }@for (membership of memberships(); track membership.id) { <option [value]="membership.id">{{ membership.planName }} · {{ membership.studentId }} · {{ membership.status }}</option> }</select>@if (invalid('membershipId')) { <span class="field-error">Selecciona una membresía.</span> }</div>
