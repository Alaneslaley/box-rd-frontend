...
@Component({ selector: 'app-student-attendance-page', imports: [RouterLink, PageHeaderComponent, LoadingStateComponent, EmptyStateComponent, ErrorStateComponent, StatusBadgeComponent], template: `
  <app-page-header title="Historial de asistencia" description="Consulta las asistencias registradas del alumno."><div class="header-actions"><a class="btn btn-secondary" [routerLink]="['/students', studentId]">Volver al alumno</a><a class="btn btn-primary" routerLink="/attendance/check-in" [queryParams]="{ studentId: studentId }">Registrar check-in</a></div></app-page-header>
...
export class StudentAttendancePageComponent implements OnInit {
  private readonly facade = inject(AttendanceFacade); private readonly route = inject(ActivatedRoute);
